...
│   状态机驱动 Phase 循环 · 异常回环路由 · 批次限流 · 注入依赖 · 持久化    │
└──────────────────────────────────────────────────────────────────────────┘
           │                │                │                 │
           ▼                ▼                ▼                 ▼
     ┌──────────┐     ┌──────────┐     ┌──────────┐      ┌──────────┐
     │  Phases  │ ◀── │  Agents  │ ◀── │   LLM    │ ◀──  │  Memory  │
     │ 8 阶段   │     │ 9 类角色 │     │ 多提供商 │      │ 三层记忆 │
     └──────────┘     └──────────┘     └──────────┘      └──────────┘
           │                │                │
           ▼                ▼                ▼
     ┌──────────────────────────────────────────────┐
...
这些原则由以下机制共同守护：
- **Agent Contract**（`src/agents/contracts/*.yaml`）：每个 Agent 的输入白名单、Gate 白名单、禁止行为三项，由 `ReadOnlyStateView` 运行时拦截 + `test_agent_contracts.py` 静态扫描双重执行。
- **Coordinator**（`src/core/coordinator.py`）：集中处理异常回环路由与批次大小计算，使 Orchestrator 保持纯粹的状态机调度。
...
`PhaseContext` 还携带 `coordinator: Coordinator | None`，供 Phase 咨询异常路由决策（详见 [flow.md](flow.md) §2）。
...
├── memory/           # 三层记忆 · SQLiteStore · MemoryExtractorAgent · Summarizer · LayeredLoader
...
- Coordinator：`coordinator_directives: list[MetaReviewResult]`（meta-review 战略建议，供 AWAITING_HUMAN 参考）
- 记忆：`memory_db_path: str | None`（SQLite 路径，resume 时优先从 DB 恢复）
...
│   ├── judge_meta_review_threshold    # Judge 回环触发 meta-review 门槛（默认 2）
│   ├── dispute_meta_review_threshold  # Plan dispute 触发 meta-review 门槛（默认 2）
│   ├── context_utilization_ratio      # 上下文利用率，影响批次大小（默认 0.6）
│   ├── max_files_per_batch            # 批次文件硬上限（None=自动计算）
│   ├── avg_tokens_per_file            # 每文件预估 token（默认 2000）
│   └── meta_review_enabled            # 元审查总开关（默认 true）
...
1. **新增 Agent**：继承 `BaseAgent`，末尾 `AgentRegistry.register("my_agent", MyAgent)`；在 `src/agents/contracts/my_agent.yaml` 声明输入白名单、gate 白名单、禁止行为；在 `AgentsLLMConfig` 加默认配置；在 orchestrator 顶部添加 `import src.agents.my_agent  # noqa: F401`。
